...
- **Documents** - Identity documents: prefer the `documents` array; `document` and `additional_documents` are still supported (see [Create Customer](/api-reference/customers/create))
- **Tax ID** - Required when creating virtual accounts for a customer (see [Tax ID document](#tax-id-document) below)
